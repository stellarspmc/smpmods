...
def clean_json(data):
    if isinstance(data, dict):
        # 1. Remove bonus_rolls completely to fix Luck scaling issues
        if "bonus_rolls" in data:
            data.pop("bonus_rolls", None)

        # 2. Overwrite rolls with the custom uniform structure (1 to 6 rolls)
        if "rolls" in data:
            data["rolls"] = {
                "min": 1,
                "max": 6
        ...
        for key in list(data.keys()):
            if key not in ["rolls", "bonus_rolls", "weight", "conditions", "functions", "entries"]:
...
if __name__ == "__main__":
    process_directory(".")
    print("\n🎉 Completed! Rolls set to 1-6 across all files.")